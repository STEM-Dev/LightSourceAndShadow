globals [source-x film-x old-number-lights]
breed [sources]    ;light sources (1-3)
breed [light-rays] ;light particles
breed [tracers]    ;used to trace shadow lines

to setup
  clear-all
  resize-world -88 112 -40 40  set-patch-size 3
  set-default-shape sources    "circle"
  set-default-shape light-rays "ray"
  set-default-shape tracers    "tracer"
  set source-x min-pxcor + 0.1 * world-width
  set film-x 80
  make-lights
  ask patches with [(pxcor = screen-x or pxcor = screen-x + 1) and abs pycor < 5] [set pcolor red]
end

to make-lights
  clear-turtles
  let y-list [0 4 -4] ;if odd number up to 3
  if number-lights mod 2 = 0 [set y-list but-first y-list] ;modify for even number
  let j 0
  create-sources number-lights [set color white set size 2 
    setxy source-x item j y-list
    set j j + 1]
  set old-number-lights number-lights
end

to make-shadow-lines
  if count tracers > 0 [ask tracers [die]]
  ask sources [
    hatch-tracers 2 [set color red pd ]
  ask min-one-of tracers-here [who] [
    set heading atan (ycor - 5) (screen-x - xcor) + 90
    while [can-move? .1 and xcor < film-x] [fd .1]]
  ask max-one-of tracers-here [who] [
    set heading atan (ycor + 5) (screen-x - xcor) + 90
    while [can-move? .1 and xcor < film-x] [fd .1]]]
end

to go
  if max [pcolor] of patches with [pxcor = film-x] > 9.88 [stop]
  if (number-lights != old-number-lights) [make-lights]
  ask sources [hatch-light-rays 1 [set heading random-float 360]]
  ask light-rays [
    ifelse can-move? .5 [fd .5] [die]
    if ray-motion = "curve left" [set heading heading - 0.1]
    if ray-motion = "wiggle"     [set heading heading + (2 * random 2 - 1) * 0.5] 
    if (pxcor = screen-x and abs ycor < 5) [die]
    if xcor > film-x [ask patch-here [if pcolor < 9.9 [set pcolor pcolor + 0.05]] die]]
  ask patches with [pxcor = film-x + 1] [set pcolor [pcolor] of patch film-x pycor]
  update-histogram
end
 
to update-histogram 
  set-current-plot "film intensity" 
  clear-plot
  let j min-pycor
  repeat world-height [plotxy j [pcolor] of patch film-x j set j j + 1]  
end
